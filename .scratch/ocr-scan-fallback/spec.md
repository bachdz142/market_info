...
- **2026-09-02 update**: real output quality validated live against `sbv_legal_directives_official`'s scanned "CT 02_2026.pdf" — coherent, readable text recovered. The Out-of-Scope automatic hook (line 55) is still not built and still explicitly out of scope. A narrower, separate follow-on *is* now built: consuming an already-completed OCR result (not submitting a new one) through the normal structuring step, via `agent/graph.py`'s `build_ocr_structure_graph()` + the new `ocr_structure.py` CLI — see `DEVELOPMENT_PLAN.md`'s v0.19. This doesn't change anything in this spec's Out of Scope: `/trigger` still never submits or waits on an OCR job.
