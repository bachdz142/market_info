...
| v0.10 — Layer 2 (CVP/offerings), first sources + 3 real bugs found and fixed | 🚧 Partial — 5 of 10 bank news/fee sources solved (BIDV news+fee, ACB promo, VPBank news+fee, all via real API discovery); VCB/MBBank + ACB's fee page still open, all need solving per user direction (not deferred) |
