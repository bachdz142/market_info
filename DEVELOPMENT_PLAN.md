...
### Out of Scope
- The single-fetch path's auto-OCR wiring (`bidv_financial_statements` and friends) — needs the `agent/crawler.py` URL-threading refactor noted above first.
- BIDV's known worst case (a PDF with real text only on its first 2 of 56 pages) isn't caught by `check_content_usable()` at all today — clean cover-letter text doesn't trip the corrupted-token-ratio check, and isn't near-empty either. This is a real, separate detection gap (only found by the user manually opening the PDF), not something this pass's `"scan"` trigger fixes — a different check (e.g. extracted-length vs. page-count) would be needed first.
