...
**Update: the remaining 4 banks were completed the same session — see v0.26 below.**

## v0.26 — Remaining 4 banks of the Layer 3 annual-report row: VCB, BIDV, MBBank, ACB (✅ done, all verified live)

Closes out `.scratch/layer3-annual-reports/spec.md` entirely. Same method as Techcombank for each: find the real annual report PDF, verify it's a real text layer (not a scan), find its actual chapter boundaries by direct text search (not printed page numbers — most of these use a 2-printed-page-per-PDF-page spread layout), scope to leadership statements + technology disclosures + strategic direction, exclude the much larger boilerplate/governance/financial-statements chapters.

- [x] **`vietcombank_annual_report`** — VCB's own domain has a genuine, confirmed Akamai wall for Layer 1 filings (per `source_plan_mvp0.md` §8, routed to manual ingestion, not attempted here), but this PDF, served from the same `www.vietcombank.com.vn` media path already proven for VCB's Layer 2 sources, is reachable directly. Its own IR portal (`portal.vietcombank.com.vn`) times out; a guessed 2025-dated URL at the same path returned an HTML error page with a fake 200 status, not a real PDF — 2024 is the newest edition confirmed to actually exist. No dedicated Technology chapter in this report — scoped to "Vietcombank Profile" (Chairman/CEO messages) + "Report of the Board of Directors" (business performance + 2025 orientation). ~73K chars, 8 chunks.
- [x] **`bidv_annual_report`** — unlike BIDV's known scan-only Layer 1 filings, this specific annual report PDF is real text. Originally added on the FY2024 edition (91 pages), then **refreshed same-session to FY2025** once the user supplied the real page URL directly (`bidv.com.vn/bidv_en/quan-he-nha-dau-tu/bao-cao-va-tai-lieu/annualreport/2026/bctn+2025`) — the FY2024 pick was only what a first web search surfaced, not confirmed to be the newest available; a user question ("why MB bank and bidv only have annual report from 2024?") caught that this wasn't checked consistently across all 5 banks. FY2025 confirmed live: 95 pages, 402K chars, 94/95 non-empty. This year's report restructured — the standalone "Digital Banking operations" section from FY2024 no longer exists; technology content is now woven into the Management's Report chapter instead. Scoped to the Chairman's message + 4 pages of the Management's Report chapter (BOD's operational assessment, assessment of BOM activities, an executive-management assessment with real IT-operations detail, and 2026 business orientation). ~48K chars, ~5 chunks.
- [x] **`mbbank_annual_report`** — originally added via Vietstock's aggregator CDN (mbbank.com.vn confirmed Akamai-walled site-wide) on the FY2024 edition, then **refreshed same-session to FY2025** (user-supplied real page URL: `mbbank.com.vn/chi-tiet/thong-bao/bao-cao-thuong-nien-2025-...`) — this specific announcement page turned out reachable directly on mbbank.com.vn itself (its own JS-rendered PDF link), no aggregator needed. This edition is Vietnamese-only (no English variant found) — unproblematic, the LLM handles Vietnamese content natively throughout this pipeline. Confirmed live: 186 pages, 738K chars, 184/186 non-empty. Scoped to Chairman/CEO messages + the real "Chiến lược và định hướng phát triển" (Strategy and development orientation) section + "Tình hình đầu tư và thực hiện các dự án" (Project investment and implementation — 2,500+ IT staff, RPA/AI/Machine Learning/OCR applications). ~29K chars, 3 chunks.
- [x] **`acb_annual_report`** — ACB's own investors page is client-side API-rendered with no direct PDF surfaced on a plain fetch, so this also uses Vietstock. Scoped to the Chairman's Message + "1.4 Development strategy" (2025 financial targets + an explicit digital-transformation commitment) + the BOD's 2025 business-plans/vision section. ~13K chars, 2 chunks — the smallest of the 5, since ACB's report gives noticeably less space to technology specifics than the other 4 banks'.
- [x] New shared `agent/crawler.py` helper: `_fetch_annual_report_page_ranges(url, page_ranges, bank_name)` — all 5 banks' fetch functions are now thin wrappers around it (refactored out of Techcombank's original standalone implementation once a second bank needed the same logic).
...
- v0.24's 4 new cimigo/decisionlab.co sources use hand-picked, hardcoded article URLs, not live discovery — user-raised and deliberately deferred (2026-09-03): cimigo has a real, working category filter (`?category=market-research-vietnam`) that could become a live "fetch listing → click N most recent" source, same pattern as IAV/mbbank_news; decisionlab.co has no working category/tag filter, so its equivalent would be a live sitemap.xml fetch + the same keyword filter applied by hand this pass, rather than a frozen list. Deliberately not "click whatever looks meaningful on the homepage" — both sites are general marketing blogs (skincare, beer, hiring posts mixed in), so "most recent" or "most prominent" alone isn't a reliable relevance signal the way it is for IAV/mbbank_news's single-topic feeds.
