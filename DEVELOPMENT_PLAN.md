...
Securities-firm research ends this pass at 2 of 4 named firms (SSI, VCBS), not the 1 of 4 first reported — VCBS was corrected mid-pass after the user pushed back on the "blocked-by-design" conclusion and did their own manual click-through, surfacing the real working URL. Worth remembering: a JS-wait-only investigation concluding "blocked by design" isn't the same as an investigation that actually tried a genuinely trusted click.
...
- [ ] **BSC** (securities firm) — still unsolved. Its specific report-detail page (`chi-tiet-bao-cao/714250`, "Báo cáo phân tích ngành Ngân Hàng") renders the exact same generic stock-ticker dashboard content as its category-listing page, regardless of URL or a JS wait — the report body itself never loads via a plain page visit. Given VCBS's own "blocked-by-design" conclusion turned out to be wrong until a genuinely trusted click was tried, this deserves a real click-simulation pass (not yet attempted) before concluding it's actually blocked, rather than trusting the same JS-wait-only investigation that was wrong for VCBS.
...
### Verification
- [x] Import/build sanity check — `SOURCES` imports cleanly with all 4 new entries (36 total sources), no id collisions.
- [x] Every new source's full fetch → content-gate pipeline verified live via `fetch_preview.py`, fetch-only (zero LLM cost) — all 4 pass `check_content_usable()`, real current content confirmed for each.
- [x] `tests/test_content_gate.py`: 13/13 passing (12 + 1 new regression test for the `data:` URI false positive).
- [ ] Full LLM-inclusive `pytest tests/test_sources.py` **not** run for any of the 4 new sources, per the fetch-dev-no-llm-by-default direction.
