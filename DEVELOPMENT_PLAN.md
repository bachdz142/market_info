...
- [ ] **VCBS** (securities firm) — investigated live, genuinely blocked by design rather than by a simple fix. Its report list itself only resolves after clicking its "Báo cáo ngành" tab (the real "Báo cáo ngành ngân hàng 2026" entry does surface this way, tickers MBB/BID/STB/CTG/HDB/VPB confirmed) — but the individual report's own title isn't a real navigable element: clicking it does nothing (same URL, no new content afterward). Reverse-engineering its SPA's actual per-report API is a disproportionate investment for one row.
- [ ] **BSC** (securities firm) — investigated live, same conclusion. Its specific report-detail page (`chi-tiet-bao-cao/714250`, "Báo cáo phân tích ngành Ngân Hàng") renders the exact same generic stock-ticker dashboard content as its category-listing page, regardless of URL or a JS wait — the report body itself never loads via a plain page visit.
- [ ] **Cimigo** (consumer research) — skipped after checking, not gated this time but genuinely stale: its only freely-fetchable retail-banking content (both its 2022 PDF and its "evergreen" trends page, which republishes the same 2022 GDP/COVID-era figures under a non-dated URL) predates its 2024 report, which is itself email-gated (submit-to-download) and whose old landing page now 404s. Its homepage's other current (2025/2026) trend articles were also checked directly and contain zero banking/fintech-relevant content. No workaround attempted — a user decision to skip, not a technical dead end.
...
### Verification
- [x] Import/build sanity check — `SOURCES` imports cleanly with all 3 new entries (35 total sources), no id collisions.
- [x] Every new source's full fetch → content-gate pipeline verified live via `fetch_preview.py`, fetch-only (zero LLM cost) — all 3 pass `check_content_usable()`, real current content confirmed for each.
- [x] `tests/test_content_gate.py`: 13/13 passing (12 + 1 new regression test for the `data:` URI false positive).
- [ ] Full LLM-inclusive `pytest tests/test_sources.py` **not** run for any of the 3 new sources, per the fetch-dev-no-llm-by-default direction.
