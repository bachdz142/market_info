...
| v0.10 — Layer 2 (CVP/offerings), first sources + 3 real bugs found and fixed | 🚧 Partial — 6 of 10 bank news/fee sources solved (BIDV news+fee, ACB promo, VPBank news+fee, VCB promo); VCB fee needs OCR (image-only); MBBank blocked by design (Akamai wall, spec says don't evade); ACB fee genuinely still open (no consolidated fee page found, not a wall) |
...
- [ ] **Vietcombank's fee page — needs OCR, not more crawling.** The fee table is an embedded image, not JS-gapped content (confirmed earlier: a "Desktop_Biu-ph-biu-mu.jpg" reference, zero extractable text). Same category as BIDV's/Vietcombank's own Layer 1 scan-only filings — out of scope until the user's OCR work lands.
- [x] **`acb_promotions`** — solved via real Playwright network capture (not a guess): the rendered listing said "Không có sản phẩm" (no products), same AJAX-gap as VPBank, but capturing the page's actual network requests revealed a two-step API — `map/posts?type=uu-dai` lists promo ids, then each id's real content only comes back from the **Vietnamese-locale** detail endpoint (`/api/vi/front/v1/posts/{id}` — the English one returns nulls for these Vietnamese-only posts). Confirmed live: 8 real, current promotions with validity dates. ACB's fee page not yet individually re-checked (may need the same technique).
- [ ] **MBBank** (news + fee) — **blocked by design, not by lack of effort.** Re-confirmed live: every path on `mbbank.com.vn` (`/tin-tuc`, `/uu-dai`, `/bieu-phi`) returns the identical "177 bytes, 0 chars visible" WAF block already documented for Layer 1 — comprehensive and site-wide, not a JS-rendering gap the network-capture technique could solve (the page never loads enough to fire any JS at all). Per `source_plan_mvp0.md` §8's explicit rule for Akamai-walled sites ("switch entirely to the manual ingestion path — no workarounds"), not attempting evasion — same treatment as Vietcombank's own walled `portal.vietcombank.com.vn`. Vietstock's static-CDN mirror (the Layer 1 workaround) only has financial-statement PDFs, not news/promo/fee content, so it doesn't help here.
- [ ] **ACB's fee/pricing page — genuinely still open, not a wall.** The `map/*` API technique that solved ACB's promotions page doesn't transfer: guessed fee-related `type` values (`bieu-phi`, `phi-dich-vu`) all returned HTTP 422, and network capture on `/en/fees` showed no fee-specific API call. ACB's site likely doesn't have one consolidated fee-schedule listing — pricing appears scattered per-product page (cards, loans, deposits shown separately) rather than aggregated. Would need per-product-page discovery, a larger effort than this pass's remaining budget covered.
